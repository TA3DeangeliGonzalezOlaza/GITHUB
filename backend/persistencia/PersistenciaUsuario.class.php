...
class PersistenciaUsuario
{
      ...
      public function agregar($obj, $conex)
    {
		...
		$result->execute(array(":reputacion_usu" => $reputacion_usu, ":suspendido" => $suspendido, ":rol" => $rol,":premium" => $premium));
   ...
   public function consTipo($obj, $conex)
   {
		...
		return $result;
	      ...
	      public function SancionUsu($obj, $conex)
    {
   
 		$id_usu= $obj->getIdusuario();       

		$sql = "update usuario set suspendido=1 where id_usu=:id_usu";
		...
		$result->execute(array(":id_usu" => $id_usu));
	      ...
	      public function RevocaSancionUsu($obj, $conex)
    {
   
 		$id_usu= $obj->getIdusuario();       

		$sql = "update usuario set suspendido=0 where id_usu=:id_usu";
